...
const email_2 = require("../constants/email");
...
const DEFAULT_CONTENT_SECTIONS = [
    ...
    },
    {
        ...
        content: {
            ...
            description: 'We craft premium cotton tote bags in India for businesses, exporters, and corporates worldwide. Every piece is ethically sourced, GOTS certified, and built to meet the highest global standards for export-ready programmes—without compromising on quality or sustainability.',
    ...
    },
    {
        ...
        content: {
            ...
            email_primary: email_2.MAIN_EMAIL,
            email_secondary: '',
            phone: '+91 7020631149',
...
async function migrateContentImages() {
    for (const [sectionKey, fields] of Object.entries(IMAGE_MIGRATIONS)) {
        ...
        for (const [field, defaultVal] of Object.entries(fields)) {
            if (field === 'slides') {
                const slides = (0, contentNormalize_1.coerceHeroSlidesArray)(content[field]);
                if (slides.length === 0) {
...
router.post('/products', auth_1.authenticateToken, upload.any(), async (req, res) => {
    try {
        ...
        const slug = await (0, slug_1.generateUniqueProductSlug)(pool, String(name));
        const baseParams = [
            String(name),
            String(category),
            String(description),
        ...
        try {
            await pool.execute(`INSERT INTO products (id, slug, name, category, description, material, print_type, packaging, moq, price, image_url, gallery_images, specifications, is_featured, is_active) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`, [productId, slug, baseParams[0], baseParams[1], baseParams[2], baseParams[3], baseParams[4], baseParams[5], baseParams[6], baseParams[7], baseParams[8], gallery_images, baseParams[9], baseParams[10]]);
        }
        catch (insertErr) {
            if (insertErr?.code === 'ER_BAD_FIELD_ERROR' && insertErr?.sqlMessage?.includes('gallery_images')) {
                await pool.execute(`INSERT INTO products (id, slug, name, category, description, material, print_type, packaging, moq, price, image_url, specifications, is_featured, is_active) 
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)`, [productId, slug, ...baseParams]);
            }
        ...
        try {
            germanTranslation = await (0, productTranslation_1.syncProductGermanTranslation)(pool, productId);
        }
...
router.put('/products/:id', auth_1.authenticateToken, upload.any(), async (req, res) => {
    try {
        ...
        const existingSlug = Array.isArray(existingSlugRows) && existingSlugRows[0]?.slug
            ...
            : '';
        const slug = name && (!existingSlug || (0, slug_1.slugNeedsSeoUpdate)(String(name), existingSlug))
            ? await (0, slug_1.generateUniqueProductSlug)(pool, String(name), id)
            : existingSlug;
        let updateQuery = `
      UPDATE products SET 
    ...
    `;
        ...
        let germanTranslation = false;
        try {
            germanTranslation = await (0, productTranslation_1.syncProductGermanTranslation)(pool, id);
        }
        catch (translationErr) {
...
function bufferOrValueToString(v) {
...
router.get('/sample-requests', auth_1.authenticateToken, async (req, res) => {
    try {
        const [rawRows] = await pool.execute(`SELECT
         sr.id,
         sr.product_id,
         sr.product_name,
         sr.name,
...
router.put('/sample-requests/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
        const { status } = req.body;
...
async function deleteSampleRequestHandler(req, res) {
    try {
        const { id } = req.params;
        if (!id) {
...
router.delete('/sample-requests/:id', auth_1.authenticateToken, deleteSampleRequestHandler);
router.post('/sample-requests/:id/delete', auth_1.authenticateToken, deleteSampleRequestHandler);
router.get('/content', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const normalized = rows.map((row) => {
            ...
            if (!parsed)
                return row;
            const content = (0, contentNormalize_1.normalizeSectionContent)(sectionKey, parsed);
            return { ...row, content };
        });
        res.json(normalized);
    }
...
router.put('/content/:id', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const [keyRows] = await pool.execute('SELECT section_key FROM content_sections WHERE id = ?', [id]);
        const sectionKey = Array.isArray(keyRows) && keyRows.length > 0 ? String(keyRows[0].section_key ?? '') : '';
        let contentToSave = content && typeof content === 'object' && !Array.isArray(content)
            ? content
            : {};
        if (sectionKey) {
            contentToSave = (0, contentNormalize_1.normalizeSectionContent)(sectionKey, contentToSave);
        }
        await pool.execute('UPDATE content_sections SET title = ?, content = ?, is_active = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [title, JSON.stringify(contentToSave), is_active, id]);
